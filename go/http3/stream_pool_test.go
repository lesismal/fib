...
// until its response is written and its handler has returned, so its size is
// memory per request in flight: it stays within the allocator's 896-byte size class,
